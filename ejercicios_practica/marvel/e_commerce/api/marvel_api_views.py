...
def get_comics(request):
    ...
    if request.GET.get('page') == None or request.GET['page'].isdigit() == False:
        page = int(offset)//15
    else:
    ...
    for i in range(len(id)):
        ...
        <tr>
            ...
            <td>
                <form action="/e_commerce/purchased_item/" method="post" , style ="visibility: {visibility};">
    ...
    ## Cuando deshabilitamos los botones:
    disabled_prev = page == 0
    disabled_next = not comics_list # Si, por ahora solo va a hacer efecto en la página siguiente ya vacía.
    
    ## Manejamos los botones según estén habilitados o no:
    search_prev = f'href="/e_commerce/comics/?offset={offset-15}&page={page-1}"' if not disabled_prev else ""
    search_next = f'href="/e_commerce/comics/?offset={offset+15}&page={page+1}"' if not disabled_next else ""
    
    ## Damos estilo a los botones según estén habilitados o no:
    btn_styling = 'style="cursor: default; text-decoration: none;font-weight:bold;"' if disabled_next else 'style="text-decoration: none;font-weight:bold;"'
    
    template += f'''
                </table>
            </div>
            <div action="/e_commerce/comics/" method="get" , style ="display: flex; flex-direction:row; align-items:center; justify-content:center; gap:1rem;">
                <a {btn_styling} {search_prev} ><</a>
                <p style="font-weight:bold;">{page}</p>
                <a {btn_styling} {search_next}>></a>
            </div>
        </div>'''
